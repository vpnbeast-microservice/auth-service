...
func init() {
	...
	privateKey = config.GetStringEnv("PRIVATE_KEY", "-----BEGIN PRIVATE KEY-----\nMIICdwIBADANBgkqhkiG" +
...
func GenerateAccessToken(username string) (string, error) {
	...
	t.Claims = &jwt.StandardClaims{
		ExpiresAt: time.Now().Add(time.Duration(int32(accessTokenValidInMinutes)) * time.Minute).Unix(),
		Issuer:    issuer,
		Subject:   username,
...
func GenerateRefreshToken(username string) (string, error) {
	t := jwt.New(jwt.GetSigningMethod("RS256"))
	t.Claims = &jwt.StandardClaims{
		ExpiresAt: time.Now().Add(time.Duration(int32(refreshTokenValidInMinutes)) * time.Minute).Unix(),
		Issuer:    issuer,
		Subject:   username,
		IssuedAt:  time.Now().Unix(),
	}
